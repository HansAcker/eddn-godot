...
#	print(headers)
	...
	if !(_response is Array):
		print("EDSM query returned no data")
		return
	...
	for _sys in response:
		if !(_sys is Dictionary):
			print("EDSM response: array member not a Dictionary")
			continue
		...
		if !(id is int || id is float):
			print("EDSM response: no id64 in %s" % sys)
			continue
		sys_entry.id = id

		var name = sys.get("name")
		if !(name is String):
			print("EDSM response: no name in %s" % sys)
			continue
		sys_entry.name = name

		var _coords = sys.get("coords")
		if !(_coords is Dictionary):
			print("EDSM response: no coords in %s" % sys)
			continue
		...
		var x = coords.get("x")
		if !(x is float):
			print("EDSM response: invalid coords in %s" % sys)
			continue
		...
		if _primary is Dictionary:
			...
			if _star_class is String:
				...
				if !(star_class in StarClasses.colors):
					print("EDSM response: no color for star class %s (%s)" % [star_class, _star_class])
